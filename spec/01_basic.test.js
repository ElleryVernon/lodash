describe("identity", function () {
	checkVarUsed(_.identity.toString());
	checkBuiltInUsed(function () {
		_.identity(1);
	});
...
describe("filter", function () {
	...
	it("callback으로 문자열의 길이가 6이상인지 판별하는 함수를 입력받은 경우, 길이 6 이상의 문자열만을 요소로 갖는 배열을 리턴해야 합니다", function () {
		...
		const arr = ["hello", "hanghae", "javascript", "sw", "web"];
		const longs = _.filter(arr, isLargerThanEqualSix);

		expect(longs).to.eql(["hanghae", "javascript"]);
	});
